...
class InvoiceApp(ctk.CTk):
    ...
    def create_widgets(self):
        ...
        ctk.CTkLabel(top_frame, text="Data wystawienia:").grid(row=0, column=4, padx=5, pady=5, sticky="e")
        self.entry_date = ctk.CTkEntry(top_frame)
        self.entry_date.insert(0, datetime.now().strftime("%d.%m.%Y"))
        self.entry_date.grid(row=0, column=5, padx=5, pady=5, sticky="w")
        ...
        self.entry_title.grid(row=1, column=1, padx=5, pady=5, sticky="w", columnspan=2)

        ctk.CTkLabel(top_frame, text="Miejscowość:").grid(row=1, column=3, padx=5, pady=5, sticky="e")
        self.entry_city = ctk.CTkEntry(top_frame, placeholder_text="np. Warszawa")
        self.entry_city.grid(row=1, column=4, padx=5, pady=5, sticky="w", columnspan=2)
        ...
        self.entry_name.grid(row=0, column=3, padx=5, pady=5, columnspan=3)

        ctk.CTkLabel(mid_frame, text="Ilość:").grid(row=1, column=0, padx=5, pady=5)

        self.entry_qty = ctk.CTkEntry(mid_frame, width=80)
        self.entry_qty.grid(row=1, column=1, padx=5, pady=5, sticky="w")

        self.combo_unit = ctk.CTkComboBox(mid_frame, values=["szt.", "m2", "mb.", "op.", "m"], width=70)
        self.combo_unit.grid(row=1, column=1, padx=5, pady=5, sticky="e")

        ctk.CTkLabel(mid_frame, text="Cena Netto:").grid(row=1, column=2, padx=5, pady=5)
        self.entry_price = ctk.CTkEntry(mid_frame, width=100)
        ...
        self.items_scroll = ctk.CTkScrollableFrame(self, height=200)
        self.items_scroll.pack(pady=10, padx=10, fill="both", expand=True)
    ...
    def refresh_items_view(self):
        ...
        for idx, item in enumerate(self.materialy):
            self.create_item_row(item, "Materiał", idx)

        for idx, item in enumerate(self.robocizna):
            self.create_item_row(item, "Robocizna", idx)

    def create_item_row(self, item, kategoria, idx):
        row_frame = ctk.CTkFrame(self.items_scroll)
        row_frame.pack(fill="x", pady=2)

        tekst = f"[{kategoria}] {item['nazwa']} | {item['ilosc']} {item['jednostka']} | Netto: {item['w_netto']:.2f} zł | Brutto: {item['w_brutto']:.2f} zł"
        lbl = ctk.CTkLabel(row_frame, text=tekst)
        lbl.pack(side="left", padx=10)

        btn_del = ctk.CTkButton(row_frame, text="Usuń", width=50, fg_color="#dc3545", hover_color="#c82333",
                                command=lambda k=kategoria, i=idx: self.delete_item(k, i))
        btn_del.pack(side="right", padx=10)

    def delete_item(self, kategoria, idx):
        if kategoria == "Materiał":
            self.materialy.pop(idx)
        else:
            self.robocizna.pop(idx)
        self.refresh_items_view()
    ...
    def generate_pdf(self):
        ...
        data_wystawienia = self.entry_date.get()
        ...
        pdf.set_font(font_family, "", 10)
        pdf.cell(0, 6, "Nazwa firmy", new_x="LMARGIN", new_y="NEXT")
        pdf.cell(0, 6, "ADRES", new_x="LMARGIN", new_y="NEXT")
        ...
        def rysuj_tabele(tytul, lista_pozycji):
            ...
            with pdf.table(col_widths=col_widths, text_align="CENTER") as table:
                ...
                for item in lista_pozycji:
                    ...
                    row.cell(f"{item['ilosc']} {item['jednostka']}")
